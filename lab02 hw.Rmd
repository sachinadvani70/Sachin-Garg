...
```{r}
 v = 1:100 #populate the data from 1 to 100 into variable v
 my_reverse = function(v){   #create a function my_reverse which takes vector v as input
   n = length(v)     # set the vector v equals to length(v) which is 100
  vr = array(NA, n)
  for (i in 1:n){     #create a for loop from i=1 to n
    vr[i] = v[n-i+1]         
    
  }
  vr
 }
 my_reverse(v)    # return the function my_reverse()
...
```{r}
flip_matrix = function(X,dim_to_rev = NA){  #create a matrix flip_matrix
  nr=nrows(X)       # no. of rows in matrix X
  nc=ncols(X)       # no. of columns in matrix X
  if(is.na(dim_to_rev)){
    if(nr==nc){
...
```{r}
my_list = list()  # create a list 'my_list'
#my_list$A= array(1,1)
...
#To-Do The function lapply returns a list of same size as my_list 
...
```{r}
n = 14 * 3 * 2 * 3 * 10   # 14 levels in 3 variable treatments 'a,b,c', 3 submanipulations with levels 'A,B,C', 2 levels of gender 'M/F', 3 levels of generations 'Boomer,GenX,Milleneial" and 10 names for each generation. 

treatments = c("A","B", "C")  #combines the arguments 'A','B','C' in variable 'treatment'
genders = c("m","f")   #combines the arguments 'm','f' in variable genders
variations = c("a","b","c") #combines the arguments 'a','b','c' in variable variations
generations = c("Millenial","GenX","Boomer") #combines the arguments 'Millenial','GenX','Boomer' in variable generations
X = data.frame(
  treatment = rep(c("A","B","C"), each = 6*3*2*3), 
  variation = rep(rep(c("a","b","c"), each = 6*3*2), times = 3),
  gender = rep(rep(c("m","f"), each = 18), times = 3*3),
  generation = rep(rep(c("Millenial","GenX", "Boomer"), each = 6), times = 2*3*3),
  name = NA,
  y = NA
)
X        #return the dataframe X
...
#rep(rep(c("m","f"), each = 18), times = 3*3)
#rep(rep(c("a","b","c"), each = 6*3*2), times = 3)
#rep(rep(c("A","B","C"), each = 6*3*2*3))
...
```{r}
v= seq(-100, 100)      #create vector v of sequence from -100 to 100
