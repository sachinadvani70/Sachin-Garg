...
bills_data[discount_days_until_discount == 0, discount_days_until_discount := 1]
...
```{r}
train_indices = sample(1 : nrow(adult), 5000)
...
test_indices = sample(setdiff(1 : nrow(adult), train_indices), 5000)
...
```{r}
log_score = mean(ytrain * log(p_hats_train) + (1 - ytrain) * log(1 - p_hats_train))

brier_score = mean(-(ytrain - p_hats_train)^2)
```
...
brier_score = mean(-(ytrain - p_hats_train)^2)
pacman::p_load(ggplot2)
ggplot(data.frame(p_hats_train = p_hats_train, y_train = ytrain)) + 
  geom_histogram(aes(x = p_hats_train, fill = ytrain), alpha = 0.5)

p_hats_test = predict(logistic_mod, adult_test, type = "response")
ggplot(data.frame(p_hats_test = p_hats_test, y_test = ytest)) + 
  geom_histogram(aes(x = p_hats_test, fill = ytest), alpha = 0.5)
```
