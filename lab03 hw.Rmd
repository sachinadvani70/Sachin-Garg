...
set.seed(1984)
n = 20
x = runif(n)   # set x as random uniform distribution 
beta_0 = 3
beta_1 = -2
```
...
```{r}
my_simple_ols = function(x, y){  #create a function my_simple_ols which takes x and y as arguments
  ols_obj = list()    # create a list ols_obj()
  n = length(x)
  y_bar = sum(y)/n      # compute the mean of vector y
  x_bar = sum(x)/n      # compute the mean of vector x 
  s_y = sqrt(sum((y-y_bar)^2)/(n-1))  #compute standard deviation for vector x
  s_x = sqrt(sum((y-y_bar)^2)/(n-1))  #compute standard deviation for vector y
  s_xy = sum((x-x_bar)*(y-y_bar))/(n-1)  #compute standard deviation for vector x and vector y
  r = s_xy/(s_x*s_y)    
  
  b_1 = r*(s_y/s_x)     # compute slope
  b_0 = y_bar - b_1*x_bar   # compute the intercept 
  
  y_hat = b_0 + (b_1*x)   # prediction of the dataset 
  e = y-y_hat             # compute residual 
  SSE = sum(e^2)          # compute sum of squared errors
  SST = sum((y-y_bar)^2)   # compute sum of squared totals
  MSE = SSE/(n-2)         # compute mean squared errors
  RMSE = sqrt(MSE)        # compute root mean square errors
  Rsq = 1 - SSE/SST       # compute R-squared
...
my_simple_ols(x,y)        # return the function my_simple_ols
```
...
ns = 10^(1:8)
errors = array(NA, length(ns)) #or errors_in_beta
for (i in 1 : length(ns)) {
  ...
  errors[i] = (mod$b_0 - beta_0)^2 + (mod$b_1 - beta_1)^2 #sum((beta - b)^2)
}
rbind(ns,errors)                    #errors
```
...
skim(Galton)
```
#There are 928 no. of observations and two column for parent and child in the dataset. The dataset shows that parents are relatively taller than the children. It also shows that children height depends on their parents height.  

Find the average height (include both parents and children in this computation).

```{r}
y = c(Galton$parent, Galton$child)
avg_height = sum(y)/(2*nrow(Galton))  # Average height of all parents and children 
```
...
```{r}
SSE_0 = sum((y-sum(y)/n)^2)
RMSE = sqrt(SSE_0)/(2*nrow(Galton)-2)
```
...
```{r}
my_ols = function(X, y){  #create a function my_ols which takes agrument as vector X and vector y
  n = nrow(X)      # number of rows in matrix X
  p = ncol(X)      # number of columns in matrix X        
  X = cbind(1,X)    # populate column of 1's in matrix X
  Xt = t(X)         # transpose of matrix X
  XtX = Xt %*% X         # matrix X times X transpose 
  XtXinv = solve(XtX)    # matrix X times X transpose times inverse of X
  b = XtXinv %*% Xt %*% y
  y_hat = X %*% b       #vector of n predictions
  e = y - y_hat         # vector of n residuals 
  df = p + 1            # degree of freedom of model p 
  SSE = sum(e^2)         # compute sum of squared errors
  SST = var(y)*(n-1)     # compute sum of squares total
  MSE = SSE/(n-df)       # compute mean squared errors
  RMSE = sqrt(MSE)       # compute root mean squared errors
  R_sqr = 1 - SSE/SST    # compute R-square
  ols = list()           
  ols$y_hat = y_hat
  ols$e = e
  ols$df = df
...
```{r}
g = function(x_star, X, y){    #create a function g which takes argument as vector x-star, vector X and vector y
  X = cbind(1,X)     # populate column of 1's in matrix X
  Xt = t(X)          # compute transpose of matrix X
  XtX = Xt %*% X       # compute X times transpose of X
  XtXinv = solve(XtX)
  b = XtXinv %*% Xt %*% y
  cat(length(b), "\n")
  cat(length(x_star), "\n")
...
g(X[7,],X,y)        #returns the function g 
```
